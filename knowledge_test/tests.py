...
class QuestionTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(title="Информатика", description="Про компьютеры и т.п.")
        self.course_test = CourseTest.objects.create(name="Тест по информатике", course=self.course)
        self.question = Question.objects.create(question_text="Доколе?", course_test=self.course_test)
...
class AnswerTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.course_test = CourseTest.objects.create(name="Тест по информатике", course=self.course)
        self.question = Question.objects.create(question_text="Доколе?", course_test=self.course_test)
        self.answer = Answer.objects.create(question=self.question, answer_text="Потому что", is_correct=False)
